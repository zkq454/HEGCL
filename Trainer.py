...
from model import HEGCL
from CL import Classifier, UCL, WCL, PNCL
...
class Trainer(object):
    def __init__(self, params):
        ...
        self.pncl_temp = params.pncl_temp
        self.alpha = params.alpha
        self.beta = params.beta
        self.para = params.para
        self.theta = params.theta
        self.newloss = params.newloss
        ...
        if self.dataset_name == 'MR' or self.dataset_name == 'Twitter' or self.dataset_name == 'TagMyNews':
            self.ucl_temp = 0.7
            self.wcl_temp = 0.7
            self.pncl_temp = 0.7
        elif self.dataset_name == 'Snippets':
            self.ucl_temp = 0.1
            self.wcl_temp = 0.1
            self.pncl_temp = 0.1
        else:
        ...
        self.model = HEGCL(self.adj_dict, self.features_dict, self.in_features_dim, self.out_features_dim, params)
        ...
        self.pncl = PNCL(self.in_features_dim[-1], self.hidden_size, self.pncl_temp, self.para)
        self.pncl = self.pncl.to(self.device)
    ...
    def train(self):
        ...
        for i in range(1, self.max_epoch + 1):
            ...
            pncl_loss = self.pncl(doc_fea, doc_fea_svd)
            loss = cls_loss + self.beta * wcl_loss + self.newloss * pncl_loss + self.alpha * ucl_loss
